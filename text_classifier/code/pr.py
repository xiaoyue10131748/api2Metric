...
class ABLSTM(object):
    def __init__(self, config):
        ...
        self.perms_words=["permission sgid gid uid suid security permission capable capabable perm privilege privileged permissions securityfs cap permission"]
        self.UI_words=["user userspace mount namespace unmounting mode tty"]
        #self.AC_words = ["must check verify validate determine acquire request revalidate require requires lock security capable access NetLabel gid sgid rights semaphore flag futex credential encryption"]
        #self.keywords = keyword_preprocessing_v2(self.perms_words,max_len=50)
        self.keywords = keyword_preprocessing_v3(self.perms_words,self.max_len)
...
def train():
    ...
    x_train, y_train, api_train = load_data("../data/PR/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test, api_test = load_data("../data/PR/test.csv", one_hot=False)
    sentence, _, api = load_data("../data/PR/test.csv", one_hot=False)
    # data preprocessing
    ...
    model.save("./matix_model/PR_model")
    att_model.save("./attention/PR_atten_weights")
    print("\n\n\n\n\n\n\n")



def test():
    x_train, y_train, api_train = load_data("../data/PR/linux/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test, api_test = load_data("../data/PR/linux/test.csv", one_hot=False)
    sentence, _, api = load_data("../data/PR/linux/test.csv", one_hot=False)
    # data preprocessing
