...
class ABLSTM(object):
    def __init__(self, config):
        ...
        self.AC_words = ["capability validity check verify validate determine acquire acquired request revalidate require requires lock  access NetLabel gid sgid rights semaphore futex credential encryption mutex hwspinlock"]
        #self.keywords = keyword_preprocessing_v2(self.perms_words,max_len=50)
        self.keywords = keyword_preprocessing_v3(self.AC_words,self.max_len)
...
def train():
    ...
    x_train, y_train, api_train = load_data("../data/AC/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test, api_test = load_data("../data/AC/test.csv", one_hot=False)
    sentence, _, api = load_data("../data/AC/test.csv", one_hot=False)
    # data preprocessing
    x_train, x_test = data_preprocessing_v3(x_train, x_test, max_len=140)
    ...
    model.save("./matix_model/AC_model_v3")
    att_model.save("./attention/AC_atten_weights")
    print("\n\n\n\n\n\n\n")



def test():
    x_train, y_train, api_train = load_data("../data/AC/linux/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test, api_test = load_data("../data/AC/linux/test.csv", one_hot=False)
    sentence, _, api = load_data("../data/AC/linux/test.csv", one_hot=False)
    # data preprocessing
    x_train, x_test = data_preprocessing_v3(x_train, x_test, max_len=150)

    print("second evaluating:  \n")
    AV_model = k.models.load_model("./matix_model/AC_model_best")
    rst1 = AV_model.predict(fill_feed_dict_once(x_test, y_test, 1), verbose=1)
    rp(y_test, rst1, sentence, api)


def testall():
    sentence, _, api = load_data("../data/AC/android/total_ac_android.csv", one_hot=False)
    x_train, y_train, api_train = load_data("../data/AC/linux/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test, api_test = load_data("../data/AC/android/total_ac_android.csv", one_hot=False)
    ...
    df.to_excel("../data/AC/android/ac_android_4.9.xlsx")
    print(count)
